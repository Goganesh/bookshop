...
import com.goganesh.bookshop.webui.exception.NoSuchAuthorPageException;
...
public class BooksAuthorPageController {
    ...
    public String booksAuthorPage(@PathVariable(value = "slug", required = true) String slug,
                                  Model model) {
        Author author = authorRepository.findBySlug(slug).orElseThrow(() -> new NoSuchAuthorPageException("No such authr with slug - " + slug));
